function  [ctdbtl,insituSal,SALdata,Fname3,PN_fig] = Stage_7b_mbtl_insitu_SAL_matchup_unique_Sensor(manual_outlier_removal, DeploymentInfo, ctdbtl, insituSal, Fname3)
%
%
% for the selected cruise, matches up the in situ bottle salinity sample with the
% corresponding .btl data, when only one of two T and S sensors are
% available.
% input data:
...
%
% Date created: 23/10/2015....## Still a work in progress
%
...
%             end
    ...
    hold on
    scatter(1:length(SALdata.insitu.Calculated_Salinity),resid_Sal00,86,SALdata.btl.Sal00,'o','filled')
    hbar=colorbar;
    CMi=37;CMa=38.8;
    caxis([CMi,CMa])
    ...
    legend('Sensor00','mean residual Sal00','Location','best')
    grid on
...
%     ylim([-0.1,0.34])
    ...
    imwrite(img.cdata, [PN_fig,DeploymentInfo.deploymentName,'/','SalResid_beforeCorrection_',Fname3, '.png']);
    ...
    if manual_outlier_removal==0
        ...
        rmX = questDialogManualOutlierRemoval(SALdata, resid_Sal00, Fname3,PN_fig);
        ...
        img = getframe(gcf);
        imwrite(img.cdata, [PN_fig,DeploymentInfo.deploymentName,'/','SalResid_beforeCorrection_Manual_outliersCircled',Fname3, '.png']);
        
        h = figure('Position',[50 50 scrsz(3)-300 scrsz(4)-150]);
        scatter(1:length(SALdata.insitu.Calculated_Salinity),resid_Sal00.*indTOkeep,86,SALdata.btl.Sal00,'o','filled')
        hbar=colorbar;
        caxis([CMi,CMa]);
        cmap=colormap(jet(length(CMi:0.1:CMa)-1));
        set(hbar,'ytick',CMi:0.1:CMa)
        ...
        img = getframe(gcf);
        imwrite(img.cdata, [PN_fig,DeploymentInfo.deploymentName,'/','SalResid_beforeCorrection_MANUALoutlierREM_',Fname3, '.png']);
    end
...
%     indTOkeep(salSensorDiff > 0.01) = nan;
    ...
    legend('Sensor00','mean residual Sal00','Corrected Sensor00','mean residual Sal00','Location','best')
    %print(gcf,[PnamFIG,'SalResid_with_without_correction_',Fname3{1}],'-dpng','-r0')
    img = getframe(gcf);
    imwrite(img.cdata, [PN_fig,DeploymentInfo.deploymentName,'/','SalResid_with_without_correction_',Fname3, '.png']);
...
%     plot(1:length(SALdata.insitu.Calculated_Salinity),resid_Sal11_COR.*indTOkeep11,'db','markersize',10,'markerfacecolor','b')
    hold on
    scatter(1:length(SALdata.insitu.Calculated_Salinity),resid_Sal00_COR.*indTOkeep00,86,SALdata.btl.Sal00,'o','filled')
    hbar=colorbar;
    caxis([CMi,CMa])
    cmap=colormap(jet(length(CMi:0.1:CMa)-1));
    set(hbar,'ytick',CMi:0.1:CMa)
    ...
    legend('Corrected Sensor00','mean residual Sal00','Location','best')
    grid on
    ylabel('residual salinity (in situ - sensor)','fontname','arial','fontsize',16,'fontweight','bold')
    set(gca,'fontsize',16,'fontweight','b')
    ...
    img = getframe(gcf);
    imwrite(img.cdata, [PN_fig,DeploymentInfo.deploymentName,'/','SalResid_Corrected_',Fname3, '.png']);
